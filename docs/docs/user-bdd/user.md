...
## 👨‍💻 假如我开始拖动组件，舞台上的 Flex 组件应该变化为可见样式 {#drag-component-flex-visible}
...
## 👨‍💻 假如我拖动组件进入部件相邻后一个占位插槽放置后，应该可以看到该组件插入到正确的位置中 {#insert-slot-next}
...
## 👨‍💻 假如用户点击舞台上的部件后，侧边栏应该展示可配置信息 {#click-component-show-config-panel}
...
## 👨‍💻 假如用户拖动组件进入部件后，应该根据鼠标在部件内的位置选择靠近的占位插槽进行展示 {#drag-component-select-closest-slot}
...
## 👨‍💻 假如用户拖动组件悬停组件上时，相邻插槽应该有鼠标触发的渐变效果 {#drag-hover-slot}
