...
describe('insertNode', () => {
  it('应该将节点插入到正确的父节点并返回 true', () => {
    ...
    const result = 插入节点(initialTree, 2, newNode, 0);
  ...
  it('应该在指定位置插入节点并返回 true', () => {
    ...
    const newNode: TreeNode = { key: 4, children: [] };
    const result = 插入节点(initialTree, 2, newNode, 1);
    expect(result).toBe(true);
  ...
  it('如果父节点不存在应该返回 false', () => {
    ...
    const result = 插入节点(initialTree, 999, newNode, 0);
  ...
  it('如果父节点没有 children 数组应该创建一个新的 children 数组并返回 true', () => {
    ...
    const result = 插入节点(initialTree, 1, newNode, 0);
    expect(result).toBe(true);
  ...
  it('当 position 为负数时应该抛出错误', () => {
    ...
    const newNode: TreeNode = { key: 4, children: [] };
    expect(() => 插入节点(initialTree, 2, newNode, -1)).toThrow(
      'Index out of bounds.',
    );
  });
  ...
  it('当 position 超出范围时应该抛出错误', () => {
    ...
    const newNode: TreeNode = { key: 4, children: [] };
    expect(() => 插入节点(initialTree, 2, newNode, 10)).toThrow(
      'Index out of bounds.',
    );
  });
});
